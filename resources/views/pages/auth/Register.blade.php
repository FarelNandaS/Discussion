        ...
        <form action="{{ route('register-attempt') }}" method="post" class="space-y-4">
          ...
          <div class="form-control w-full">
            ...
            <input type="text" name="username" id="username" value="{{ old('username') }}" placeholder="johndoe"
              class="input input-bordered focus:input-primary w-full @error('username') input-error @enderror" required
              autofocus>
            @error('username')
              <label class="label">
          ...
          <div class="form-control w-full">
            ...
            <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="name@example.com"
              class="input input-bordered focus:input-primary w-full @error('email') input-error @enderror" required>
            @error('email')
              <label class="label">
                <span class="label-text-alt text-error font-medium">{{ $message }}</span>
              </label>
          ...
          <div class="form-control w-full">
            ...
            <div class="relative">
              <input type="password" name="password" id="password" placeholder="••••••••"
                class="input input-bordered focus:input-primary w-full pr-12 @error('password') input-error @enderror" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#]).{8,}$"
                required>
              <button type="button"
  ...
  <script>
    ...
    password.addEventListener('input', () => {
      ...
      Object.keys(reqs).forEach(key => {
        ...
        const element = reqs[key].el;

        if (isValid) {
          element.classList.remove('text-base-content/50', 'text-error');
          element.classList.add('text-success', 'font-medium');
        } else {
          element.classList.remove('text-success', 'font-medium');
