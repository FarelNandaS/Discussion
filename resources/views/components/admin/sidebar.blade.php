  ...
  <ul class="flex flex-col mt-6">
    <li>
      <a href="{{ route('admin-dashboard') }}"
        ...
        <x-tabler-layout-dashboard style="width: 25px; height: 25px;" /> <span>Dashboard</span>
      </a>
    </li>
    <li>
    ...
    @if (auth()->user()->hasRole('Super Admin'))
      <li>
        <a href="{{ route('admin-user-management') }}"
          class="flex items-center gap-2 p-2 pl-4 rounded-none lg:text-lg is-drawer-close:tooltip is-drawer-close:tooltip-right {{ str_contains($currentRoute, 'admin-user-management') ? 'menu-active' : '' }}">
          <x-tabler-users-group style="width: 25px; height: 25px;" /> <span>User Management</span>
        </a>
      </li>
    @endif
  </ul>
